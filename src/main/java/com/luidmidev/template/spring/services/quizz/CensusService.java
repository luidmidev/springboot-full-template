...
import com.luidmidev.template.spring.services.quizz.models.BlockQuestion;
...
public class CensusService {
    ...
    public CensusService(QuestionAnswerRepository repository) {

        this.repository = repository;

        this.questions = of(

                new BlockQuestion("Información general", of(
                ...
                new BlockQuestion("Información de plantación", of(
    ...
    public void saveAnswers(List<Answer> responses) {

        log.info("Guardando respuestas del usuario {}", responses);

        var mesesAltos = getAnswer(22L, responses).getValue();
        var mesesBajos = getAnswer(23L, responses).getValue();

        if (mesesAltos instanceof List<?> mesesAltosList && mesesBajos instanceof List<?> mesesBajosList) {
            if (mesesAltosList.stream().anyMatch(mesesBajosList::contains)) {
                throw new ClientException("Los meses altos y bajos de producción no pueden ser los mismos");
            }
        } else {
            throw new ClientException("Debe seleccionar al menos un mes para los meses altos y bajos de producción");
        }

        for (var question : questions) {
            for (var response : responses) question.validateAnswer(response);
        }

        log.info("Respuestas validadas correctamenteaaa");
        var questionAnswers = new QuestionAnswers();
        questionAnswers.setAnswers(responses);
        questionAnswers.setCreationDate();
        repository.save(questionAnswers);
        log.info("Respuestas guardadas correctamente");
    }

    private Answer getAnswer(Long questionId, List<Answer> answers) {
        return answers
                .stream()
